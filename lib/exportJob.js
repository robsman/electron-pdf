...
class ExportJob extends EventEmitter {
  ...
  _launchBrowserWindow () {
    ...
    let session = electron.session.defaultSession;
    const requestFilter = {
      urls: []
    };

    session.webRequest.onBeforeSendHeaders(requestFilter, (details, callback) => {
      console.info('Outgoing request: ' + JSON.stringify(details));

      const loadOpts = this._getLoadURLConfiguration(this.args);
      const extraHeaders = {};
      loadOpts.extraHeaders.split('\n').forEach(header => {
        let splitIdx = header.indexOf(':');
        if (-1 === splitIdx) {
          splitIdx = header.length;
        }
        let name = header.substring(0, splitIdx);
        let value = header.substring(splitIdx + 1);
        extraHeaders[name] = value;
      });
      console.log(`Extra headers: ${JSON.stringify(extraHeaders)}`);

      Object.keys(extraHeaders).forEach(name => {
        details.requestHeaders[name] = extraHeaders[name];
      });
  ...
  /**
   * see
   * http://electron.atom.io/docs/api/browser-window/#winloadurlurl-options
   * @param args
   * @returns {Object} for BrowserWindow#loadURL
   * @private
   */
  _getLoadURLConfiguration (args) {
    const defaultOpts = {
      extraHeaders: ''
    }
    let cmdLineLoadURLConfig = {}
    try {
      cmdLineLoadURLConfig = JSON.parse(args.loadURLConfig || '{}')
    } catch (e) {
      errorLogger('Invalid loadURLConfig provided, using defaults. Value:',
        args.loadURLConfig,
        '\nError:', e)
    }
    return _.extend(defaultOpts, cmdLineLoadURLConfig)
  }
   ...
   */
  _loadURL (window, url) {
    const loadOpts = Object.assign({}, this._getLoadURLConfiguration(this.args))
